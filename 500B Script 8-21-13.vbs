...
Dim messtxt,z,Row,WshShell,rowcheck,QMStatus,note1,StrPickle,test1
'************Ask for data file
...
Do
rowcheck = ExcelSheet.Cells(Row+1,1).Value 
Call MMextend500B
Call ext99
Call extsubc
Call TaxClassCheck
...
session.findById("wnd[0]").sendVKey 0
'row=row+1
test1=ExcelSheet.Cells(Row,1).Value
...
'session.findById("wnd[1]/usr/tblSAPLMGMMTC_VIEW").getAbsoluteRow(18).selected = true
...
If session.findById("wnd[0]/usr/tabsTABSPR1/tabpSP11/ssubTABFRA1:SAPLMGMM:2010/subSUB2:SAPLMGD1:2321/cntlLONGTEXT_BESTELL/shellcont/shell").text="" Then
