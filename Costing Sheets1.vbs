...
Sub extractBomWithCosts
	...
	ws.Cells(row+5,7).Formula = "=((Sum(G3:G"&row+2&"))*.03+G"&row+4&")"
...
Sub GetComponentCosts
	...
	End If
	On Error Resume next
	If session.findbyid("wnd[1]").text="Class Type 4 Entries" Then
		session.findbyid("wnd[1]/tbar[0]/btn[0]").press
		session.findbyid("wnd[0]/tbar[1]/btn[8]").press
	End If
	If session.findbyid("wnd[0]/titl").name ="Classification" Then
		session.findbyid("wnd[0]/tbar[1]/btn[8]").press
	End If
